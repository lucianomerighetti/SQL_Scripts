select * 
    FROM ( 
      select 
        ...
        wb.dat_transfering 
      FROM overload.atm_adm_atm_withdraw_batch wb 
      INNER JOIN overload.atm_adm_config_withdraw_batch cwb on wb.idt_config_withdraw_batch = cwb.idt_config_withdraw_batch 
      LEFT JOIN overload.atm_adm_withdraw w on wb.idt_atm_withdraw_batch = w.idt_atm_withdraw_batch 
      WHERE 1=1
      AND flg_closed= 0 
       AND (wb.dat_due is null  
       OR wb.dat_due >= to_date('26/06/2021','dd/MM/yyyy') AND wb.dat_due <= to_date('05/07/2021','dd/MM/yyyy') )
    ...
    UNION ALL 
      ...
      FROM overload.atm_adm_atm_withdraw_batch wb 
      INNER JOIN overload.atm_adm_config_withdraw_batch cwb on wb.idt_config_withdraw_batch = cwb.idt_config_withdraw_batch 
      LEFT JOIN customer_cashout.atm_adm_atm_withdraw_batch_summary wbs on wbs.idt_atm_withdraw_batch = wb.idt_atm_withdraw_batch 
      WHERE flg_closed = 1 
