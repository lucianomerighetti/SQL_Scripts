...
VCOUNT NUMBER := 0;
C_ROLLBACK VARCHAR2(2000) := 'ALTER TABLE RECEIVABLES_ADM.RECEIVABLE_INTENT DROP CONSTRAINT JSN_PAYLOAD_REC_INTENT_CK';
C_SQL VARCHAR2(2000) 	  := 'ALTER TABLE RECEIVABLES_ADM.RECEIVABLE_INTENT ADD CONSTRAINT JSN_PAYLOAD_REC_INTENT_CK CHECK ( JSN_PAYLOAD IS JSON ) ENABLE NOVALIDATE';
BEGIN
    SELECT COUNT(1)
    INTO VCOUNT
    FROM ALL_CONSTRAINTS
    WHERE CONSTRAINT_TYPE = 'C'
    AND OWNER = 'RECEIVABLES_ADM'
