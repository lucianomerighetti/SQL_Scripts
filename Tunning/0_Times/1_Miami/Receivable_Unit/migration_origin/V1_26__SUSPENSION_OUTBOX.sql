CREATE TABLE RECEIVABLES_ADM.SUSPENSION_OUTBOX (
    IDT_SUSPENSION_OUTBOX      NUMBER(20) GENERATED ALWAYS AS IDENTITY INCREMENT BY 1 MAXVALUE 9999999999999999999999999999 MINVALUE 1 CACHE 1000 NOCYCLE NOORDER NOT NULL,
...
COMMENT ON TABLE RECEIVABLES_ADM.SUSPENSION_OUTBOX IS
    'Tabela de outbox de suspenções para replicar no kafka';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.IDT_SUSPENSION_OUTBOX IS
    '[NOT_SECURITY_APPLY] Identificador unico e sequencial da tabela';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.IDT_PAYMENT_SUSPENSION IS
    '[NOT_SECURITY_APPLY] Identificador do registor da suspenção';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.dat_partition IS
    '[NOT_SECURITY_APPLY] Dia da criacao do registro usado para particionar a tabela';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.dat_creation IS
    '[NOT_SECURITY_APPLY] Data e hora da criacao do registro na tabela';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.cod_customer IS
    '[NOT_SECURITY_APPLY] Codigo identificador unico do customer que sera usado como chave de particao no kafka';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.ind_status IS
    '[NOT_SECURITY_APPLY] Status do envio do registro ao kafka, W - waiting, S - sent, E - error';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.num_topic_offset IS
    '[NOT_SECURITY_APPLY] Offset corrente da mensagem no topico';

COMMENT ON COLUMN RECEIVABLES_ADM.SUSPENSION_OUTBOX.jsn_metadata IS
    '[NOT_SECURITY_APPLY] Binario do conteudo da mensagem a ser postado no kafka';

CREATE INDEX RECEIVABLES_ADM.SUSPENSION_OUTBOX_IDX01 ON RECEIVABLES_ADM.SUSPENSION_OUTBOX
...
CREATE INDEX RECEIVABLES_ADM.SUSPENSION_OUTBOX_IDX02 ON RECEIVABLES_ADM.SUSPENSION_OUTBOX
(IDT_PAYMENT_SUSPENSION) TABLESPACE TSIRECEIVABLES01 INITRANS 48 ONLINE;

GRANT SELECT,UPDATE,INSERT,DELETE ON RECEIVABLES_ADM.SUSPENSION_OUTBOX TO RECEIVABLESUBR;
GRANT SELECT ON RECEIVABLES_ADM.SUSPENSION_OUTBOX TO MANUT_RECEIVABLES;