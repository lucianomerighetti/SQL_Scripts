/*
AUTOMATIZACAO DE SCRIPT DE MOVES -- MOVERA TODOS OS SEGMENTOS DE UMA TABELA QUE ESTEJA EM DETERMINADAS TABLESPACES
*/
...
DECLARE
  ...
  P_TABLE VARCHAR2(4000) := 'REACHED_RECEIVABLE_UNIT'; -- NOME DA TABELA DA OPERACAO
  P_TSDORIG VARCHAR2(4000) := 'TSDSETTLEMENTCONTRACT01'; -- TABLESPACE DE DADOS ORIGEM
  P_TSIORIG VARCHAR2(4000) := 'TSISETTLEMENTCONTRACT01'; -- TABLESPACE DE INDICE ORIGEM
  P_TSLORIG VARCHAR2(4000) := 'TSLSETTLEMENTCONTRACT01'; -- TABLESPACE DE LOB ORIGEM   
  P_TSDDEST VARCHAR2(4000) := 'TSDSETTLEMENTCONTRACT23'; -- TABLESPACE DE DADOS DESTINO
  P_TSIDEST VARCHAR2(4000) := 'TSISETTLEMENTCONTRACT23'; -- TABLESPACE DE INDICE DESTINO
  P_TSLDEST VARCHAR2(4000) := 'TSLSETTLEMENTCONTRACT01'; -- TABLESPACE DE LOB DESTINO 
  ...
  P_DISABLE_FK CHAR(1) := 'N'; -- OPCAO DE DESATIVAR FKS
        ...
        IF P_UPDATE_INDEX = 'N' THEN
          ...
          FOR R3 IN C_INDEXES(R1.TABLE_OWNER, R1.TABLE_NAME, R1.PARTITION_NAME) LOOP
            ...
            IF TO_DATE(V_DAT_PART,'YYYY-MM-DD') < P_DAT_CORTE_COMPRESS THEN	  
              ...
              V_MOVE_IND := '-- '||R3.INDEX_NAME||' -- '||R3.PARTITION_NAME||' -- '||R3.SIZE_GB||' GB '||CHR(10)||'ALTER INDEX '||R3.INDEX_OWNER||'.'||R3.INDEX_NAME||' REBUILD PARTITION '||R3.PARTITION_NAME||' TABLESPACE '||NVL(P_TSIDEST,R3.TABLESPACE_NAME)||V_PARAM_COMPRESS||V_PARAMS;
